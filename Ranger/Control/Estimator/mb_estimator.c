/* To READ code BETTER in Kyle: click on all '-' signs, on left, hence shrinking all blocks of code */

#include <mb_includes.h> // a file that includes all the other .h files from all other codes. 
                        // This is a convenient way to manage the large number of headers, which might be changing all the time. 

// ///////////////////////////////////////////////////
// //////// sign conventions and notations  ///////////
// //////////////////////////////////////////////////

//1) hip angle is the angle between two legs and is positive when inner leg is in front (assuming robot is walking forwards)
//2) ankle angle is angle between leg-line and the line joining the ankle joint to the centre of foot-circle (the foot is part of a circle). 
//   It is positive when ankle is infront of leg (i.e. eg when foot isflipped down)
//   The ankle angle can-ids measure the physical angle of encoder which is zero when foot is (almost fully) flipped up, so we have  
//   ankle_angle = measured from can id - ankle_angle_offset (defined as parameter = 96.2 deg)
//3) all absolute angles (imu or legs, feet) are measured from vertical and are positive if the line making the angle is slanting forwards. 
//   So if the direction of the robot's forward motion is x, and y is oppositve to gravity, then all abolute angles are measured from y with clockwise as positive.
//4) angles rate are just angle dots. imu-rate as given by the imu in its current state of mounting on outer leg is negative in my convention. 
//   Hence I put a negative sign whenever I read that CAN-ID. 
//5) In the code below: L = leg, F = foot, I = inner, O = outer, HS = heel strike = hs,  ang = angle, abs = absolute (or magnitude),
//   prev = previous (variable from last time the function was called), curr = current (variable updated during the current function call)
//   stnc = t = stance,  swng = w = swing, L = length, d = distance, a = ankle joint,  c = center (of the foot circle)
//   E = estimation,  H = hip


// THE MAIN FUNCTION of this file which calls all other functions here is mb_estimator_update. It inturn is called by the scheduler.
// This code assumes at many places that the main brain runs the code every 2ms (eg in the counters in leg state machine)
// This code assumes that the robot walks in the forward motion. 
// !!!!!!!!!!!!! PLEASE REGULARLY CHECK and update the gyro-bias variable, it changes with time !!!!!!!!!!!!!!!!!!!!

/* ///////////////////////////////////////////////////
//////////global variables and parameters ////////////
/////////////////////////////////////////////////// */

static int prev_ui_fsm_state = 0; // the can ID ID_FSM_RESET changes to 2 when the robot goes to walking state, after for example the reset button was pressed.
static int curr_ui_fsm_state = 0; // I need to reset some variables when the robot starts a new segment of its walk. Resetting happens in mb_estimator_update

 
static float current_LO_absang_by_integration; // LO absang estimated by integrating the imu-rate 'manually'.
static float prev_LO_absang_by_integration = 0; // angle from last time the code was run
static float prev_StncFootAbsAngl=0;  // stance foot's previous abs angle (used to compute how much distance the robot moved due to rolling of its foot which is on the ground) (no sliding assumed)

//These static variable for:  mb_leg_state_machine_old(), but many are still in use.
const unsigned static int  above_ground = 0 , on_ground = 1 ; 
const unsigned static int  inner_leg_swings =1 , outer_leg_swings =3; //  double_stance= 2;      // state for the robot leg                                      
static volatile unsigned int est_leg_state_old_version = inner_leg_swings, est_prev_leg_state = inner_leg_swings; // state for the robot leg.
...
static int inner_sense_reset = 0,     outer_sense_reset = 0;     //flags which tell that it's time to reset the filter which computes the average sensor values from the foot contact sensors
static int inner_take_baseline = 0, outer_take_baseline = 0; //flags which tell that it's time to store the average value of above filters. When the foot hits the ground that instant is detected as the sensor value going a particular threshhold above this average. 
static unsigned int counter1=0, counter2=0; // counter for counting how many units (unit = 2ms = main brain freq) has the contact sensor's value been above the threshold. 

//Some flags and counters
static volatile unsigned int reset_integration_flag = 0, reset_time_after_HS_flag = 0 ; // flags telling that HW has happened and its time to reset the imu angle based on joint angle, and its time to reset the counter which tells the time since the last HS
static int step_counter = 0;            // this is updated when the time since last HS is set.
static float  distance_travelled = 0;   // it is updated where the integration is reset.

//various geometry parametes
static const float ankle_flip_down_threshold = 0.8 ;  // if can id for ankle sesor (not the ankle angle, in my convention) gets larger than this it means foot has flipped down
...
static const float d_ankle2sweetspot = .0586; //.054 (old value before matt changed the feet, feb 18 2013); //  // distance between ankle joint and the sweet spot in the foot, this distance I trust within +- 0.0005 meters 
static const float foot_radius_outer  = .1729; // 0.168 (old value feb 18 2013) ; // foot radius 20 // this is dubious because it's hard to measure its from 15-22cm, 
static const float d_ac_outer = 0.168-.054; //d_ac=ankle length = distance from ankle_joint to foot_circle's center.  //its being written as FootRadius-d_ankle2sweetspot, I TRUST the ankle2sweetspot (0.054 m) MORE
static const float foot_radius_inner  = 0.2344; // 0.23 (old value feb 18 2013) ; // foot radius 20 // this is dubious because it's hard to measure its from 15-22cm, 
static const float d_ac_inner = 0.23-.054; // d_ac average for the two inner feet.  

static const float ankle_angle_offset = 1.66; //1.68 (old value feb 18 2013);//  // ankle_angle = value measured from can id - offset (ankle angle is between leg and the ankle-joint-2-foot-centre line, foot being thought as a big circle). add pi to this ankle_angle and you get the angle to sweet spot's angle 

// for imu
static float gyro_bias =-.0077; // -.004 (old value feb 18 2013)
...
// Read the documentation for the working of the filters. They are basically operating at the 1ms sampling time even though main brain
// runs every 2ms, because the least count of time stamp is 1, the missing data is assumed to same as the previous data (this is called zero order hold).
// input to the filter is data stream = filter_in, output is the data stream called filter_out
// filter_n = value at n-th time step, filter_n_1 = value at previous (n-1) time step ( 1 ms before) , filter_n_2 = value 2ms before. 
...
// DO FILTER 
delta_t = time_of_curr_read_data - time_of_prev_read_data ; // considering that the filter runs every 1ms, this variable tells me
// how many data points spaced at 1ms are missing, these data points will be assumed to same as the last read data and filter will be run delta_t number of times. 
// to make up for the fact that it was supposed to run that many times but the main brain only calls it every 2ms or so. 
// delta_t is not always 2, it is sometimes 1, sometimes 3 sometimes even more. This is why the sampling time of filters is chosen to be 1ms, which is the least count of delta_t. 
if (delta_t < 0){
    
    } 
    ...
    while(i < delta_t) // this is loop accounting for the missed filter runs. 
      { filter_in_n = prev_read_data;
        ...
        mb_io_set_float(ID_E_OUTER_ANG_RATE, filter_out_n);  // this sets the estimated data to the estimation id.
...
// this function registers when the heel strike happens and updates which leg is swinging and which is stance, 
// and sets various flags. 

// the main idea to detect the HS is, as soon as the swing legs crosses the other leg to go in front start averaging the value of foot contact sensor, 
// this averaging is to find the mean-foot sensor vlues when foot is in air, and is accomplished by a simple first order filter. For coding simplicity the 
// filter is running continuously, but when the swing leg crosses it is reset so that it gives the current average by the time the foot is about to hit. 
// when the foot is about to hit (based on when the ankle angle is sufficient) we store the mean value so far, and we call it the baseline. 
// compare the foot sensor value to baseline, if its above by a particular threshold for counter_max number of times for either of the two feet. call it a HS.

const unsigned int counter1_max = 2, counter2_max = 2; // number of times to wait on the signal to make decision about foot status
float filter_factor = 0.9; 
float threshhold_high = 2000.0; 
//float threshhold_low  = 1500.0;
float max_baseline_possible = 5000.0; // if baseline is above this then the sensor is considered bad or stuck. 
...
float outer_ankle_angle = mb_io_get_float(ID_MCFO_RIGHT_ANKLE_ANGLE); // just the can id value not the ankleangle of my convention. 
...
//mb_io_set_float( ID_E_TEST8 , mean_fo_right); // output the data to a canid , for testing. 
...
// state stuff
 ...
 {  // reset inner senser filter which finds the mean sensor value when the foot is in air. 
        if ((hip_angle>0.005)&&(inner_sense_reset == 0)) {
            inner_sense_reset = 1; 
            mean_fi_right = mb_io_get_float(ID_MCFI_RIGHT_HEEL_SENSE); // this resets the filter to whatever the current sensor value is. 
            ...
            fi_right_baseline = mean_fi_right; // store the mean value of foot in air, so far.
            ...
            if ( (mb_io_get_float(ID_MCFI_RIGHT_HEEL_SENSE))>=(threshhold_high+fi_right_baseline) )
                ...
                mb_io_set_float( ID_E_TEST4 , (float) fi_right_state); // this line can be commented out
                }
            ...
            if ( (mb_io_get_float(ID_MCFI_LEFT_HEEL_SENSE))>=(threshhold_high+fi_left_baseline) )
                ...
                mb_io_set_float( ID_E_TEST5 , (float) fi_left_state ); // this line can be commented out
                } 
            ...
            if (((fi_left_state == on_ground)&&(fi_left_baseline<max_baseline_possible)) || ((fi_right_state == on_ground)&&(fi_right_baseline<max_baseline_possible)))
                ...
                reset_integration_flag = 1;  // this tell that its time to current the absolute angle being integrated from the imu rate.
                reset_time_after_HS_flag = 1; // this tells that its time reset the counter which tells how much time has passed since last HS.
                mb_io_set_float( ID_E_TEST3 , est_leg_state_old_version);
                mb_io_set_float( ID_E_SWING_LEG , 0.0);  // 0 means outer, 1 means inner 
                mb_io_set_float( ID_E_DBL_STANCE, 1.0); // 1 means it's a double stance phase. 
 ...
 {  // reset outer sense
        if ((hip_angle<0)&&(outer_sense_reset == 0)) {
            ...
            if ( (mb_io_get_float(ID_MCFO_RIGHT_HEEL_SENSE))>=(threshhold_high+fo_right_baseline) )
                ...
                mb_io_set_float( ID_E_TEST6 , (float) fo_right_state);  // this line can be commented out
                }
            ...
            if ( (mb_io_get_float(ID_MCFO_LEFT_HEEL_SENSE))>=(threshhold_high+fo_left_baseline) )
                ...
                mb_io_set_float( ID_E_TEST7 , (float) fo_left_state);  // this line can be commented out
                } 
            ...
            if (((fo_left_state == on_ground)&&(fo_left_baseline<max_baseline_possible)) || ((fo_right_state == on_ground)&&(fo_right_baseline<max_baseline_possible) ))
                ...
                mb_io_set_float( ID_E_TEST3 , est_leg_state_old_version);
                mb_io_set_float( ID_E_SWING_LEG , 1.0);  // 0 means outer, 1 means inner 
                mb_io_set_float( ID_E_DBL_STANCE, 1.0); // 1 means it's a double stance phase. 
...
if ( reset_time_after_HS_flag == 1 ) // when the swing foot hits the gorund. This flag is set in leg_state_machine
...
if ((time_since_HS >= 50) && (time_since_HS <= 60))     // 2nd consition is to please Anoop (and not execute the actions many times)
{ mb_io_set_float( ID_E_DBL_STANCE, 0.0); // 1 means it's a double stance phase. 
}
...
//variables
static float         int_angle_error_at_hs = 0; // error in angle integrated from the imu rate.
...
float         hip_angle = mb_io_get_float(ID_MCH_ANGLE);
float outer_ankle_angle = mb_io_get_float(ID_MCFO_RIGHT_ANKLE_ANGLE)- ankle_angle_offset; // the offset is defined in parameters at the top is this file.
...
// integrate the imu rate to get the LO abs ang. Note that integration is by the mid point rule and it assumes that time stamps are measured in units of ms, hence the factor 0.0005; 
...
// reset integration every hs by geometrically calculating absangs using only the joint angles, when all four feet are on ground (i.e when the heel strike occurs) 
 if ( reset_integration_flag == 1 ) // when the swing foot hits the gorund
  {      reset_integration_flag = 0;  // extinguish the flag. This flag is set in the state machine again at next HS
         // swing_foot_velocity=0 ; 
         
         step_counter++; // increment the step counter. at the HS. 
         mb_io_set_float(ID_E_STEP_NO, (float)step_counter); // number of steps so far.
         
         LO_absang = mb_io_get_float(ID_E_LO_ABSANG);  // absolute angles so far
         LI_absang = LO_absang - hip_angle;
         FO_absang = LO_absang + outer_ankle_angle; 
         FI_absang = LI_absang + inner_ankle_angle;
         
         /////////////////////////////////////////////////////
         // FROM HERE ON: abbreviation: stnc and enclitic: t stand for stance, and swng = w = swing. 
         // see the diagram in documentation for clarity. its basically two lines representing legs forming an inverted v. the two feet are liek two circles resting on ground. 
         // The end points of the leg lines (called ankle joints) are at some distance away from the foot centers. 
         // hip angle is anglebetween legs postive when inner leg is in front. ankle_angle is angle between leg line and the line joining
         // ankle joint to the foot center, postive when this line is in front of the leg line. 
         ////////////////////////////////////////////////////
         
         if (hip_angle >= 0) 
              {
                StncAnklAngl = inner_ankle_angle;    // stance leg = new stance leg = leg which just hit and was swinging till now, its the leg in front for forward walking. 
                SwngAnklAngl = outer_ankle_angle;
                abs_HipAngl  = hip_angle;
                d_atct = d_ac_inner; // distance from stance leg's angle joint to the foot's center = ankle eccentricity. 
         ...
         abs_HipAngl_by2 = abs_HipAngl/2; // just to save division by 2 so many times.. 
         
         d_aa = 2*L_leg*greg_sin(abs_HipAngl_by2);           // d_aa= distance between inner and outer ankle joints
         ...
         StncFootAbsAngl = greg_atan((d_atct - d_atcw*CosSigma)/(d_atcw*SinSigma ));  // new change done. because when stance foot is near vertical, cmplmnt_StncFootAbsAngl = near pi/2 and its tangent is huge and when huge number isfed to greg_atan there is large error in that range. Now instead I feed (stancefootangl) to greg tan, which is close to 0 and accurate
                                                                                    // plus the sign is taken care of, I dont have to do an if loop

         // now correction for the inclination of line d_cc  (this line is inclined with respect to horizontal due to differences in foot radius)
         d_cc =  greg_sqrt(d_atct*d_atct + d_atcw*d_atcw - 2*d_atct*d_atcw*CosSigma) ;  // centre to centre distance of foot circles. used in calculating distance travelled.
         d_cc_inv = 1/d_cc; // one of the few float divisions in this code, trying to minimize the instances of this division. 
         angle_d_cc = anoop_asin((foot_radius_t - foot_radius_w)* d_cc_inv);
         StncFootAbsAngl = StncFootAbsAngl - angle_d_cc; // correct the stance angles because the foot radii are different, hence d_cc is not horizontal. and so far the ansolute angles geometry assumed that the foot radii are the same. 
         ...
         // d_stance_foot_roll = foot_radius_w*(SwngFootAbsAngl-prev_StncFootAbsAngl) ; // distance moved by the previous stance foot's (which is now considered swing after the hs) center due to foot rolling during its stance phase till now
         ...
         // this formula gives a better distance experimentally. maybe because the d_ankle2sweetspot is more accurate than foot_radius_w.
         ...
         prev_StncFootAbsAngl = StncFootAbsAngl;  // as of now I want to put the static variable prev_StncFootAbsAngl = 0; ie the foot hits the ground in sweet spot. Which is not necessarily true. But since after hitting the foot rocks a lot
                                // its hard to find what is the final stance foot abs ang. And taking the prev_stancefootabsangl as the stance foot abs angle at the collision (HS) is not a good idea.
                                // This needs some testing though, so maybe the next user can make these changes. 
         ...
         mb_io_set_float(ID_E_INT_LO_ABSANG_RESET , int_angle_error_at_hs ); /// difference between abs ang by integration and by the geometry.   
         
         
         //*********** HERE I RESET !!! *************//
        if (step_counter <= 1){ // first time counter hits
         current_LO_absang_by_integration =   current_LO_absang_by_integration - int_angle_error_at_hs; // first step fully reset based on geometry, because I don't know if the initial conditionvalue from which the integration started happening is correct or not.
                                                                                                  // initial condition is right now 0 for integration but this is not correct if the robot is not turned on with the outerleg in vertical position.
         } else {
         current_LO_absang_by_integration =   current_LO_absang_by_integration - .1*int_angle_error_at_hs; // 0.1 represent that the final angle = 90% of imu integrtion says + 10% of what the joint angle angles an geometry say. 
                                                                                                          // This number was found rounded off from experiments and may not be the most optimal. 
...
// recycle the previous stuff (for use in the integration in the beginning of this function)
 prev_LO_absang_rate  = current_LO_absang_rate;
 prev_time = current_time;
 prev_LO_absang_by_integration = current_LO_absang_by_integration;

////*********************
// FROM NOW ON FOR ALL SWH CALCULATION ABSOLUTE ANGLES ARE MADE RELATIVE TO SLOPE OF THE GROUND ////////////
////***********************
...
if((int)mb_io_get_float(ID_E_SWING_LEG) == 1 )
      {// inner leg swings 
        ank_2_ank_height        =                      L_leg*(greg_cos(LO_absang) -greg_cos(LI_absang))  ;  // vertical distance between two ankles (this goes close to 0 at HS because both the feet are in the sweet spot position approximately,and the foot radii are approximatley equal. 
        ank_height              = ank_2_ank_height   + foot_radius_outer -d_ac_outer*greg_cos(FO_absang) ; // height of the ankle
        swing_foot_height       = ank_height         - foot_radius_inner +d_ac_inner*greg_cos(FI_absang); // height of lowest point of foot circle
        material_swt_spt_height = ank_height         - d_ankle2sweetspot*greg_cos(FI_absang) ; // height of the sweet spot on the swingin foot.      
        virtual_swt_spt_height  = ank_height         - d_ankle2sweetspot;  // height of the sweet spot of the foot assuming that the foot will hit in the sweet spot position, which it might not. 
...
//               time_2_hs_counter = 0; 
//               swing_foot_velocity = filter_coeff1*swing_foot_velocity - filter_coeff2*(swing_foot_height-prev_swing_foot_height);// - because velocity is negative, filtering because the velocity is so noisy due to numerical differentiation. 
...
/*  // THIS IS AN EXPERIMENTAL FUNCTION, trying to implement model based state estimation, using an simple observer. 
...
void mb_estimator_update(void) { // main function called by SCHEDULER every row (approx 2ms as of 1 Jan 2011)
 ...
 // reset the estimation variables whenever the robot transitions to walk state from some other state.
 // and hence is about to begin a new segment of its walk. 
  ...
  mb_hip_rate_filter();
  mb_hip_motor_rate_filter(); // motor rate is obtained from the encoder in the motor.
  mb_imu_rate_filter();
 ...
 mb_leg_state_machine_new (); // do heelstrike detection better using the direct data from heel strike sensors.
  ...
  mb_time_since_last_HS_and_step_counter(); // time since last HS and how many steps the robot has moved. 
  mb_abs_leg_and_ankle_angles_and_swingfootheight(); // the most important function which calculates the distance travelled by the robot. 
                                                     // calculates the swing foot heights used to predict the HS in adavance
                                                     // and most importantly calculated the absolute angles, by integrating the imu rate and
                                                     // partially resetting it at every heel strike to account for gyro/integration errors. 
                                                     // We are able to reset because at HS, all the feet are on ground and hence the absolute angles 
                                                     // can be calculated by just geometry and the relative joint angles (joint angle sensors are pretty accurate)
...
//Utility functions  (called during other functions, optimized for speed, no division of floats)
