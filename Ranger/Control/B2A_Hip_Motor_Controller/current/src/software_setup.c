...
const VOID_VOID_F main_schedule[]=
{
  ...
  task_every_row, can_tx_battery_power, dn_sched_done,NULL, // Reverted MPK's change 6/20/16 -- MWS. No longer using current at main brain.
...
void run_occasionally(void)
{
  ...
  else if (i == 2){can_tx_motor_current();}        // Switched with motor current on March 4, 2016 by MPK
  //else if (i == 2){can_tx_battery_power();}        // Switched with motor current on March 4, 2016 by MPK % Commented by MWS 6/20/16. Transferred back to priority schedule.
  ...
  else if (i == 5){can_tx_battery_voltage();}     // Send motor controller input power over the CAN bus
  else if (i == 6){can_tx_motor_velocity();}      //Send hip angular rate as calculated from motor encoder
  else if (i == 7){dn_hip_motor_encoder_rezero();}//Adjust hip motor encoder offset slightly to keep aligned with hip joint encoder
  else if (i == 8){can_tx_exec_time();}     
  else if (i == 9){can_tx_max_exec_time();}   
