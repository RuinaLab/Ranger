...
float command_current;
   ...
   {
      command_current = get_io_float(ID_A_H_PM_A0)*(1.0 - get_io_float(ID_A_H_PM_A1)*(get_io_float(ID_E_T_AFTER_HS)/300.0)); 
   } 
   else 
   {
      command_current = 0.0;
...
{
  float discrete_hi_aftermid_dcurrent1, discrete_hi_aftermid_dcurrent2;
  ...
  {
    discrete_hi_aftermid_dcurrent1 = -get_io_float(ID_DC_H_AM_H_RATE)*(robot_hip_velocity_mid_stance - get_io_float(ID_DP_H_AM_H_RATE) );
    discrete_hi_aftermid_dcurrent2 = -get_io_float(ID_DC_H_AM_H_DRATE)*(robot_hip_velocity_mid_stance - get_io_float(ID_DP_H_AM_H_DRATE)*robot_velocity_mid_stance);
...
float command_current=0.0;
   ...
   if (get_io_float(ID_E_T_AFTER_HS) < 600.00)    
   {  
       if (get_io_float(ID_E_T_AFTER_HS) < 300.00)
       {
          command_current = get_io_float(ID_A_H_PM_A0)*(1.0 - get_io_float(ID_A_H_PM_A1)*(get_io_float(ID_E_T_AFTER_HS)/300.0))
                           +discrete_hi_aftermid_dcurrent; //Inner leg swing faster, so put in less energy
       } 
       else 
       {
...
{
  float discrete_ho_aftermid_dcurrent1, discrete_ho_aftermid_dcurrent2;
  ...
  if (((int) get_io_float(ID_D_H_ON)) == 1)
  {
    discrete_ho_aftermid_dcurrent1 = -get_io_float(ID_DC_H_AM_H_RATE)*(robot_hip_velocity_mid_stance - get_io_float(ID_DP_H_AM_H_RATE) );
    discrete_ho_aftermid_dcurrent2 = -get_io_float(ID_DC_H_AM_H_DRATE)*(robot_hip_velocity_mid_stance - get_io_float(ID_DP_H_AM_H_DRATE)*robot_velocity_mid_stance);
