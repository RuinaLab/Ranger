...
{ 
    ...
    float robot_velocity_mid_stance  = get_io_float(ID_E_MIDSTANCE_LEGRATE);
    if (fi_HS_occurred == 0 && ((int)get_io_float(ID_E_SWING_LEG) == 1) ) 
    {
      fi_HS_occurred = 1;
      ...
      set_io_float(ID_E_TEST10,fi_prepush_dangle);
    }
    else
       set_io_float(ID_E_TEST10,fi_prepush_dangle); 
  
    fi_prepush_time =  fi_prepush_time + 2.0; //the function is called once per 2 ms.  
...
{ 
   ...
   fi_afterpush_time =  fi_afterpush_time + 2.0; 
