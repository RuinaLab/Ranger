extern int stop_walk_on_inner_flag;
...
enum states 
{
  ...
  STATE_FO_stopstance, //Robot stops
  STATE_FO_startafterpush //Starting from stopping in double stance
};
...
enum sensor 
{
  ...
  COND_FO_stance_to_stopstance, //Get to stopping from stance
  COND_FO_prepush_to_stopstance, //Get to stopping from prepush
  COND_FO_stopstance_to_startafterpush //Start from stop stance to start afterpush (if robot stops)
};
...
{}
 ...
 {
 if ( (get_io_float(ID_E_LO_ABSANG) > get_io_float(ID_P_F_PP_L_ABSANG)) && (stop_prepush_flag == 0) )    // pushoff initiation condition                                                                       
  {
  ...
  if ( stop_walk_on_inner_flag  == 1 )    // Stop foot
  {
    sensor_array[COND_FO_stance_to_stopstance] = 1;
    sensor_array[COND_FO_prepush_to_stopstance] = 1;
  }
  ...
  {
    sensor_array[COND_FO_stopstance_to_startafterpush] = 1;
  } 
