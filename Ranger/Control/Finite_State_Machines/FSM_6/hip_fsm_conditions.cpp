...
//using namespace std;
extern float hi_premid_counter,  ho_premid_counter;
...
{}
 ...
 // if ((get_io_float(ID_E_STANCELEG_ABSANG)) > 0.0) //Temp based on IMU_Roll, 4/24/2010
  if ((get_io_float(ID_MCH_ANGLE)) < 0.0) //Remove once IMU starts working, 4/4/2010
  {
...
//                                                                          // condition that includes also the relevant angular velocities.
  if ((get_io_float(ID_MCH_ANGLE)) < -get_io_float(ID_P_H_EHOLD_ANG)) //Hold angle at end of swing, temp 4/27/2010
  {
...
//                                                                          // condition that includes also the relevant angular velocities.
  ...
  if (get_io_float(ID_MCFI_LEFT_HS) + get_io_float(ID_MCFI_RIGHT_HS) >= 1.0)
  {
   ...
   if (hi_premid_counter>=get_io_float(ID_H_TEST3) )
  {
    sensor_array[COND_HI_premid_to_HO_premid] = 1;
  }
  
  if (ho_premid_counter>=get_io_float(ID_H_TEST3) )
  {
    sensor_array[COND_HO_premid_to_HI_premid] = 1;
  }
  if (0)
  {
