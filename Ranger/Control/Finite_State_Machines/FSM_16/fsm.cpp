...
//      int active_sensor_array[m_num_sensors];
      ...
      if (possible_transition_counter == 0) {     // stay in current state 
   	     //cout << "->" << m_name << ": no active transition condition" << endl;
         //cout << "->" << m_name << ": executing action for state " << m_current_state << endl;
         if (m_phase_in_state_array[m_current_state] == 1){     // if first iteration of this fsm => do the entry and main actions
         ...
         else if (m_phase_in_state_array[m_current_state] == 2){    // if not first iteration of this fsm => do only the main action
            if (m_action_array[m_current_state] != NULL) {
               //cout << "->" << m_name << ": main action for state " << m_current_state << endl;
               m_action_array[m_current_state](); 
            }
      ...
      else {    // at least one of the state transition conditions is met; we have to move to a different state
      
        if (possible_transition_counter == 1){    // exactly one state transition condition is met
           m_current_sensor_input = active_sensor_array[0];
        }
  
        else {    // multiple state transition conditions are met => we have to choose which one to use, i.e. which state to transit to;
                  // use the transit condition which is defined earliest (using the 'trans_def' function) -- this information is stored in the 'm_transition_insertion_history' variable
        ...
        if ((m_phase_in_state_array[m_current_state] == 1)&&
         ...
         // if first iteration in current state, and new state = current state => do entry and main actions for current state
  	        if (m_init_action_array[m_current_state] != NULL) {
                //cout << "->" << m_name << ": start action for state " << m_current_state << endl;
                m_init_action_array[m_current_state]();
            }  
         ...
         (m_state_transition_matrix[m_current_state*m_num_sensors+m_current_sensor_input] != m_current_state)){
         // if first iteration in current state, and new state != current state => do entry, main and exit actions for current state
  	        if (m_init_action_array[m_current_state] != NULL) {
                //cout << "->" << m_name << ": start action for state " << m_current_state << endl;
                m_init_action_array[m_current_state]();
            }  
         ...
         (m_state_transition_matrix[m_current_state*m_num_sensors+m_current_sensor_input] == m_current_state)){
         // if not first iteration in current state, and new state = current state => do main action for current state
             if (m_action_array[m_current_state] != NULL) {
                 //cout << "->" << m_name << ": main action for state " << m_current_state << endl;
                 m_action_array[m_current_state](); 
            }
         ...
         (m_state_transition_matrix[m_current_state*m_num_sensors+m_current_sensor_input] != m_current_state)){
         // if not first iteration in current state, and new state != current state => do exit action for current state
             if (m_exit_action_array[m_current_state] != NULL) {
                 //cout << "->" << m_name << ": exit action for state " << m_current_state << endl;
                 m_exit_action_array[m_current_state](); 
             }
       ...
       // if transitioning to a different state => do entry and main actions for the new state
           m_current_state = m_next_state;
           m_phase_in_state_array[m_current_state] = 1;
           if (m_exit_state_array[m_current_state] == 1)
               m_ready_to_exit_flag = 1;
           
           // communicate new state information
           *m_state_communication_variable = m_current_state;     // added for faster state info update // Petr, Jan/16/2013
