...
extern fsm* leg_fsm;
extern fsm* rock_hip_fsm;
extern fsm* rock_foot_inner_fsm;
extern fsm* rock_foot_outer_fsm;
...
void all_rock_fsm_run(void)
{

    rock_hip_fsm->run();
    if ( (int) get_io_float(ID_MB_ROCK_HIP_FSM_STATE)!= g_rock_hip_fsm_state)
      set_io_float(ID_MB_ROCK_HIP_FSM_STATE, (float)g_rock_hip_fsm_state);
   
    rock_foot_inner_fsm->run();
    if ( (int) get_io_float(ID_MB_ROCK_FI_FSM_STATE) != g_rock_foot_inner_fsm_state)  
      set_io_float(ID_MB_ROCK_FI_FSM_STATE, (float)g_rock_foot_inner_fsm_state);
    
    rock_foot_outer_fsm->run();
    if ( (int)get_io_float(ID_MB_ROCK_FO_FSM_STATE)!= g_rock_foot_outer_fsm_state )
      set_io_float(ID_MB_ROCK_FO_FSM_STATE, (float)g_rock_foot_outer_fsm_state);
  
}

int ACTION_UI_rock_entry(void)
{

//PUT ROCK entry ACTIONS HERE
//reset the counter
set_io_float(ID_P_R_ROCK_TIMER,0);
set_io_float(ID_P_R_ROCK_TO_WALK, 0);
////
...
int ACTION_UI_rock(void)
{

//PUT ROCK ACTIONS HERE
all_rock_fsm_run();
//  clear_UI_LCD(2);
//  set_UI_LCD("M: R", 1);
////
