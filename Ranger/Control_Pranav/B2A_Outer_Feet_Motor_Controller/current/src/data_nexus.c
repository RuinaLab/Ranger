...
void dn_safety(void){
  ...
  int cable_stretched = ((ankle_angle - dn_get_motor_pos_rads()) > 0.3);
//  int cable_stretched = ((ankle_angle - dn_get_motor_pos_rads()) > 0.3) || 
                        //((dn_get_left_ankle_pos_rads() - dn_get_motor_pos_rads()) > 0.3); 
                         //cable stretched by 0.3 rads

  int neg_angle = ankle_angle < 0.2; //0.2 radians
  int pos_angle = ankle_angle > 2.9; //3.2 = physical limit, 0.2 radians off
  
  //Error reporting
  if (right_ls || left_ls){error_occurred(ERROR_MCFO_ANKL_LS);}
  if (neg_angle){error_occurred(ERROR_MCFO_ANKL_LO);}
  if (pos_angle){error_occurred(ERROR_MCFO_ANKL_HI);}
  if (cable_stretched){error_occurred(ERROR_MCFO_CBL_STRCH);}
...
#define DN_MC_OFFSET (0)
#define DN_MC_GAIN (-0.00152f)
...
// Heel strike detection code with hysteresis.
// For best operation the upper and lower switching points should be
// set individually for each foot sensor. The sensor signal must exceed
// the upper switch point to turn on, and be below the lower switch point
// to turn off again.
...
// ************************* Left foot ankle angle (from CAN bus)
float dn_left_ankle_pos_rads;
float dn_get_left_ankle_pos_rads(void)
{
  return dn_left_ankle_pos_rads;
}

void dn_set_left_ankle_pos_rads(float pos)
{
  dn_left_ankle_pos_rads = pos;
}

/******** OUTSIDE FEET MOTOR ENCODER REZERO *******************************/
//Over time, this function resets the motor incremental encoder value to the absolute encoder value
void dn_outer_feet_motor_encoder_rezero(void)
{
  ...
  if ((dn_get_ankle_pos_rads() < 0.3) && (dn_get_ankle_vel_rads() < 0.01) && (dn_get_ankle_vel_rads() > -0.01))
  {
    ...
    qdc_tmr0_cap01_set_angle_float(motor_position + (correction * 0.2));  //Correct some of the error each time
  }
}
...
int dn_get_ankle_position(void){
	return -1*ae_get_pos(dn_ae);
}

float dn_get_ankle_pos_rads(void){
  float pos = -1*ae_get_pos(dn_ae);
  pos = (pos) * AE_TO_RADS;
  return pos;
}
...
float dn_get_dummy_1(void){return mc_get_stiffness();}
float dn_get_dummy_2(void){return mc_get_dampness();}
float dn_get_dummy_3(void){return mc_get_command_current();}
float dn_get_dummy_4(void){return 0.0;}
