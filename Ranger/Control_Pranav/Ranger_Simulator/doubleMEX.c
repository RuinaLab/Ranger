...
#include <stdio.h>
#include <stdlib.h>
#define nmat2 14 //should be equal to state space else error 10/12/2009
#define NMAT 16 //should be atleast nmat+1, here nmat = 6
...
//GLOBAL variables... DEFINED IN doublestanceMEX.cpp ... DO NOT USE THIS FUNCTION WITHOUT THAT ONE -- IT WILL CAUSE ERRORS
...
void double_stance(double *f, double *y, double *Ry, double *Py)
{
...
Vh = Ih*R + V0*tanh(Ih*eps_f) + G_h*K*u3;
Va = Ia*R + V0*tanh(Ia*eps_f) + G_a*K*u6;
Vb = Ib*R + V0*tanh(Ib*eps_f) + G_a*K*u5;
...
Td_b = -C_sw*(u1+u2-u3-u4);//-C_sw*u4;//-C_sw*(u1+u2-u3-u4);
