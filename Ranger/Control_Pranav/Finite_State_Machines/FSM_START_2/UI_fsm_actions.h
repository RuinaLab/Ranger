...
//set_UI_LED(5, 'g');
    ...
    rock_hip_fsm->run();
    if ( (int) get_io_float(ID_T_TEST1)!= g_rock_hip_fsm_state)
      set_io_float(ID_T_TEST1, (float)g_rock_hip_fsm_state);
   
    rock_foot_inner_fsm->run();
    if ( (int) get_io_float(ID_T_TEST2) != g_rock_foot_inner_fsm_state)  
      set_io_float(ID_T_TEST2, (float)g_rock_foot_inner_fsm_state);
    
    rock_foot_outer_fsm->run();
    if ( (int)get_io_float(ID_T_TEST3)!= g_rock_foot_outer_fsm_state )
      set_io_float(ID_T_TEST3, (float)g_rock_foot_outer_fsm_state);
