...
{}
 ...
 else
 {
  if (1)
  {
    sensor_array[COND_FOOT_OUTER_stance_to_pushoff] = 1;
  }
  ...
  {
    sensor_array[COND_FOOT_OUTER_pushoff_to_flipup] = 1;
  }

  if ((get_io_float(ID_MCH_ANGLE)) < -0.15)
  {
    sensor_array[COND_FOOT_OUTER_flipup_to_flipdown] = 1;
  }
  
  if (get_io_float(ID_MCFO_LEFT_HS) + get_io_float(ID_MCFO_RIGHT_HS) >= 1.0)
  {
    sensor_array[COND_FOOT_OUTER_flipdown_to_stance] = 1;
  }
  
  if (get_io_float(ID_UI_BUTTONS) != 0.0)
  {
        sensor_array[COND_FOOT_OUTER_stop] = 1;
  }
