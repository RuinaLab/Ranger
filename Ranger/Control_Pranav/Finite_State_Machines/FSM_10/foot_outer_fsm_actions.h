extern float fo_speed_factor;


static float fo_stance_to_prepush_angle;
static float discrete_fo_prepush_dgain;
static float fo_stopstance_to_startafterpush_angle;

static int fo_start_afterpush_time_counter;
...
int ACT_FO_stopstance_entry()
{
  float target_angle;

  //target_angle = get_io_float(ID_P_F_ST_F_TANG) + get_io_float(ID_MCFO_MOTOR_POSITION) - get_io_float(ID_E_FO_ABSANG); //Use IMU
  target_angle = get_io_float(ID_P_F_SST_F_TANG) - get_io_float(ID_MCH_ANGLE) * 0.5;;   //Do not use IMU
...
int ACT_FO_stopstance()
{
  ...
  //target_angle = get_io_float(ID_P_F_SST_F_TANG) + get_io_float(ID_MCFO_MOTOR_POSITION) - get_io_float(ID_E_FO_ABSANG); //Use IMU
  target_angle =  get_io_float(ID_P_F_SST_F_TANG) - get_io_float(ID_MCH_ANGLE) * 0.5; //Do not use IMU
...
int ACT_FO_startafterpush_entry()
{


  //fo_stopstance_to_startafterpush_angle = get_io_float(ID_MCFO_RIGHT_ANKLE_ANGLE); //save the stance angle during exit
  //fo_start_afterpush_time_counter = 0;

 
  return 1;
}

int ACT_FO_startafterpush()
{ 
   /* float target_angle;
    target_angle = fo_stopstance_to_startafterpush_angle + get_io_float(ID_P_F_AP_F_TANG); 
   ...
   set_io_float(ID_MCFO_COMMAND_CURRENT, command_current);
      ...
      if (fo_start_afterpush_time_counter <= get_io_float(ID_F_TEST2) )
    {
           set_io_float(ID_MCFO_STIFFNESS, 0);
           set_io_float(ID_MCFO_DAMPNESS, 0);   
           set_io_float(ID_MCFO_COMMAND_CURRENT, get_io_float(ID_F_TEST1) );
           
           fo_start_afterpush_time_counter = fo_start_afterpush_time_counter + 2;
           fo_stopstance_to_startafterpush_angle = get_io_float(ID_MCFO_RIGHT_ANKLE_ANGLE);
    }
    else if ( (fo_start_afterpush_time_counter > get_io_float(ID_F_TEST2) ) 
               &&
              (fo_start_afterpush_time_counter < get_io_float(ID_F_TEST3) ) 
             )
    {
            target_angle = fo_stopstance_to_startafterpush_angle; //get_io_float(ID_P_F_AP_F_TANG);
            set_io_float(ID_MCFO_STIFFNESS, get_io_float(ID_C_F_AP_F_ANG));
...
int ACT_FO_prepush_entry()
{
  ...
  else
  {
     ...
     set_io_float(ID_MCFO_STIFFNESS, get_io_float(ID_C_F_PP_F_ANG)*fo_speed_factor+discrete_fo_prepush_dgain ); 
     set_io_float(ID_MCFO_DAMPNESS, get_io_float(ID_C_F_PP_F_RATE)*fo_speed_factor);
...
{ 
    float target_angle;
    target_angle = fo_stance_to_prepush_angle + get_io_float(ID_P_F_PP_F_TANG)*fo_speed_factor; 
    /*if (target_angle>=2.6) //Feet safety
      {target_angle=2.6;}
    if (target_angle<=1.0)
       {target_angle = 1.0;}  */
       
   set_io_float(ID_MCFO_STIFFNESS, get_io_float(ID_C_F_PP_F_ANG)*fo_speed_factor); 
   set_io_float(ID_MCFO_DAMPNESS, get_io_float(ID_C_F_PP_F_RATE)*fo_speed_factor);
   float command_current = target_angle * get_io_float(ID_MCFO_STIFFNESS);
   set_io_float(ID_MCFO_COMMAND_CURRENT, command_current);
...
int ACT_FO_startstance()
{
  ...
  set_io_float(ID_MCFO_STIFFNESS, get_io_float(ID_C_FO_SST_F_ANG));
  set_io_float(ID_MCFO_DAMPNESS, get_io_float(ID_C_FO_SST_F_RATE));
  
  target_angle =  get_io_float(ID_P_FO_SST_F_TANG) - get_io_float(ID_MCH_ANGLE) * 0.5;
