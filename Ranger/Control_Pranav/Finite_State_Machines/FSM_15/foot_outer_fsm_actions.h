...
{ 
  ...
  if (fo_HS_occurred == 0 && ((int)get_io_float(ID_E_SWING_LEG) == 0) ) //detect heel-strike in pre-push
    {
      ...
      //Added 4/25/2011
        if (((int) get_io_float(ID_D_F_ON)) == 2) 
      { 
        fo_prepush_dgain = discrete_fo_prepush_dgain;
        fo_prepush_dangle = discrete_fo_prepush_angle;      
      }     
  ...
  set_io_float(ID_MCFO_STIFFNESS, get_io_float(ID_C_F_PP_F_ANG)+fo_prepush_dgain); //Added 4/25/2011 
  set_io_float(ID_MCFO_DAMPNESS, get_io_float(ID_C_F_PP_F_RATE)); 
  
   target_angle = get_io_float(ID_P_F_ST_F_TANG)+fo_prepush_angle + get_io_float(ID_MCFO_MOTOR_POSITION)  //Added 4/25/2011 
                  - get_io_float(ID_E_FO_ABSANG) + fo_prepush_dangle; //Use IMU
