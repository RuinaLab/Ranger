...
{ 
  ...
  float robot_velocity_mid_stance  = get_io_float(ID_E_MIDSTANCE_LEGRATE);
  if (fo_HS_occurred == 0 && ((int)get_io_float(ID_E_SWING_LEG) == 0) )
    {
      fo_HS_occurred = 1;
           ...
           && (robot_velocity_mid_stance < get_io_float(ID_P_F_PP_F_LRATE)) )
      {
        ...
        set_io_float(ID_E_TEST10,fo_prepush_dangle);
    } 
    else 
      set_io_float(ID_E_TEST10,fo_prepush_dangle);
    
  fo_prepush_time =  fo_prepush_time + 2.0; //the function is called once per 2 ms.
...
{ 
  ...
  fo_afterpush_time = fo_afterpush_time + 2.0;
