...
//  FIO0CLR = (1<<20); //SSEL low
  ...
  //Initialize the PWM registers and PWM2 and PWM6 outputs
  ...
  //Writing to PWMLER bits causes selected new match register values to start being used
  PWMLER = 
  (0
    | (1<<0)    //Latch match register 0 value
    | (1<<2)    //Latch match register 2 value
    | (1<<6)    //Latch match register 6 value
  );
