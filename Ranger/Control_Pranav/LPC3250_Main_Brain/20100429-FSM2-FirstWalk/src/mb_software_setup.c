...
#define RUN_EVERY_LINE (TASK_PTR)&mb_error_update, (TASK_PTR)&mb_task_heartbeat, (TASK_PTR)&a9_dn_ssp_parse,(TASK_PTR)&mb_estimator_update, (TASK_PTR)&a9_dn_ssp_send_data, (TASK_PTR)&a9_dn_update_leds, (TASK_PTR)&mb_fsm_run, (TASK_PTR)&mb_send_data
...
// Wrapper function to read error, if any, into IO_DATA array Main Brain error location
void mb_read_error(void)
{
  mb_error_get_frame(mb_io_get_write_pointer(ID_ERROR_MB));
}
...
void mb_send_data(void)
{
  ...
  mb_io_get_unread_pointer  //pointer to function, gets pointer to data frame if new.
  );    
}
