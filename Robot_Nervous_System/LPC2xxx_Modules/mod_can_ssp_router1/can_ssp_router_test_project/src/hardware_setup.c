...
//  PCONP = (PCONP | 1<<1) & PCONP_RBM;  //Turn on Timer0 peripheral clock
...
//  PCONP = (PCONP | 1<<2) & PCONP_RBM;  //Turn on Timer1 peripheral clock
...
//  C1MOD |= (1<<2); //Self-test mode: Transmit always considered successful, even without ACK
  ...
  C3IER |= (1<<1)&0x07FF; //Turn on Tx 1 interrupt
  C3IER |= ((1<<2)|(1<<7))&0x7FF; //Turn on CAN error interrupts
//  C1IER |= (1<<2)|(1<<3)|(1<<4)|(1<<5)|(1<<6)|(1<<7); //Turn on CAN error interrupts
//  C1IER |= (1<<6); //Arbitration Lost interrupt
  C3GSR = 0;
...
//  AFMR_bit.AccOff=0;
  ...
  C4IER |= (1<<1)&0x07FF; //Turn on Tx 1 interrupt
  C4IER |= ((1<<2)|(1<<7))&0x7FF; //Turn on CAN error interrupts
//  C1IER |= (1<<2)|(1<<3)|(1<<4)|(1<<5)|(1<<6)|(1<<7); //Turn on CAN error interrupts
//  C1IER |= (1<<6); //Arbitration Lost interrupt
  C4GSR = 0;
