...
/////////////////////////////////////////////////////////////////
  ...
  SSP_CTRL = 1<<1;  // TEST enable ssp1 clock only
  ...
  SSP1CPSR = 12;     // prescale divider - set to 12 for slave mode
                     // regardless of master clock rate
 ...
 // SSP1CR1 |= 1<<0;	//Set  loopback mode test
  ...
  SSP1IMSC = 0xF;	//Enable all SSP interrupts
  //SSP1IMSC = 0;	//TEST - disable all ssp1 interrupts
...
//  long unsigned int timer1_value;
  T0IR = 0xFF;	//Clear Timer0 interrupts
  mb_schedule_tick();
//  timer1_value = T1TC;
//  T1TC = timer1_value;
  T1TC = 0;	//TEST CODE - restart timer 1
